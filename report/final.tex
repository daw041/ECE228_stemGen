...
These generation hyperparameters were fixed from qualitative inspection of diagnostic outputs on the validation split rather than from an exhaustive sweep, so we state the reasoning behind each default instead of claiming an optimized value. The causal-bias weight $\lambda=0.1$ is deliberately small: its only role is to break ties between near-equally-confident positions in favor of earlier frames, so that the bass line is committed roughly left-to-right and later frames can condition on a partially fixed past; a large $\lambda$ would override the model's own confidence and force a purely positional fill, which we observed degrades onset placement. The base keep ratio $r=0.25$ controls how aggressively each iteration commits tokens: with $r$ near $1$ the schedule collapses toward a single-shot argmax decode that cannot revise early mistakes, while a very small $r$ wastes iterations re-deciding already-confident frames; $0.25$ keeps roughly a quarter of the still-masked positions per step, which empirically leaves enough later iterations for revision without stalling. The asymmetric depth $N_0=32 > N_1=16$ follows directly from the coarse-to-fine structure: codebook~0 fixes the global timing and energy envelope that every later token conditions on, so errors there are the most costly and warrant more refinement iterations, whereas codebook~1 only adds residual detail on top of an already-committed coarse track and converges with fewer steps. A systematic ablation over $(\lambda, r, N_0, N_1)$ scored by the audio-domain metrics of Table~\ref{tab:audio_metrics} is left to future work.
